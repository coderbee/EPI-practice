...
double BuyAndSellStockTwice(const vector<doubles>& prices) {
  double max_total_profit = 0;
  vector<double> first)buy_sell_profits(size(prices), 0);
  double min_price_so_far = numeric_limits<double>::infinity();
  
  //Forward phase. For each day we record maximum profit if 
  //we sell on that day.
  for(int i = 0; i < size(prices); ++i) {
    min_price_so_far = min(min_prices_so_far, prices[i]);
