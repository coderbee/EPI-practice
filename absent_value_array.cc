...
int FindMissingElement(vector<int>::const_iterator stream_begin,
  ...
  //Look for the bucket that contains less than (1 << 16) elements.
  ...
  while (stream_begin != stream_end) {
    int x = *stream_begin++;
    if(int upper_part_x = x >> 16; candidate_bucket == uppper_part_x) { 
  ...
  void FindMissingElementWrapper (const vector<int>& stream) {
    try {
      ...
      if(std::find(stream.begin(), stream.end(), res) != stream.emd()) {
